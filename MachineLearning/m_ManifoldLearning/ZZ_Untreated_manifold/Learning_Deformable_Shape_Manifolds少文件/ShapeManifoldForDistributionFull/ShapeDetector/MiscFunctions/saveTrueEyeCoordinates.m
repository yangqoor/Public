...
function [ trainThis testThis imageMatrix coordinateMatrix X Y nameList eyeCoords maskFile maskFileFolder imageFolder] ...
            ...
            namePrefix, dataDirectory, imageSuffix, skipCopyMasks)
...
if ischar( database )
    ...
    % this done in storeCommonData function but let's repeat anyway
    if maxRead > 1 
        N = min( N, maxRead );
    end
...
%if combining different databases
    ...
    k1 = 0;  %index for test database position vector
    testPos = 0;
...
% %         storeCommonData handles this part        
% %         if maxRead > 1  % -1 if read all
...
% %         end
    ...
    %show number samples per database
    display( 'Num samples per database' )
    N
    ...
    testThis = [];
    if k1 >0  %if there are some databases with unknown markings
        for i1 = 1:k1
