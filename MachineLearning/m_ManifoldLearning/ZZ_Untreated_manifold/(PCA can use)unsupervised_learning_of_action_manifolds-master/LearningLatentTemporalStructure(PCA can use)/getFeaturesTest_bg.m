function Features_test_bg = getFeaturesTest_bg(Features,testIndex,params)
...
%     Sets = params.Sets;
    ...
    Features_test_bg = Features(:,testIndex);
    Features_HOF = cell(num_of_actions,size(Features_test_bg,2)*num_of_sets*max_num_of_subseqs);
    Features_LBP = cell(num_of_actions,size(Features_test_bg,2)*num_of_sets*max_num_of_subseqs);
    Features_RT = cell(num_of_actions,size(Features_test_bg,2)*num_of_sets*max_num_of_subseqs);

    for k = 1:1:size(Features_test_bg,1)
        for m = 1:1:size(Features_test_bg,2) % iterating through sequence
            Features_per_set = Features_test_bg{k,m};
    ...
    Features_test_bg = Features_HOF;
    [num_of_actions,num_of_videos2] = size(Features_test_bg);
   
    %Normalize the features
    for m = 1:1:num_of_actions
        for n = 1:1:num_of_videos2
            Data = Features_test_bg{m,n};
            if(length(Data) == 0)
                continue;
            end
            Data = Data ./ repmat(Norm_Factor_HOF,size(Data,1),1);
            Features_test_bg{m,n} = Data;
        end
    end
    HOF_Flow = Features_test_bg;

    % NORMALIZATION OF LBFP FEATURES
    Features_test_bg = Features_LBP;
    [num_of_actions,num_of_videos2] = size(Features_test_bg);
    ...
    LBP_Flow = Features_test_bg;

    % NORMALIZATION OF RT
    Features_test_bg = Features_RT;
    [num_of_actions,num_of_videos2] = size(Features_test_bg);
    ...
    RT_Flow = Features_test_bg;
    
    Features_test_bg = CombineFeatures(HOF_Flow,LBP_Flow); % Motion CUES
    Features_test_bg = CombineFeatures(Features_test_bg,RT_Flow);
