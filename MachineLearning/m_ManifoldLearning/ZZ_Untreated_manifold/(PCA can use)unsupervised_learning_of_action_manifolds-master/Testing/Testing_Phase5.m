...
for rr = 1
    ...
    fprintf('nt = %d\n',opts_crbm_per_action{1}.nt);
    fprintf('sizes = %d\n',opts_crbm_per_action{1}.sizes(1));
    ...
    results.action_labels_per_frame = zeros(num_of_actions,num_of_actions);
    
    est_labels_full_seq = cell(num_of_actions,num_of_test_seqs);
    est_labels_per_frame = cell(num_of_actions,num_of_test_seqs);
    est_results = zeros(num_of_actions,num_of_actions);
    tId_CRBMTest = tic;
    % Iterating through each sub-sequence
    for seq_row = 1:num_of_actions
        parfor seq_num = 1:num_of_test_seqs %1:1:(num_of_test_seqs/(num_of_sets))
...
%             test_seq = cell2mat(test_seqs_all);
            ...
            for kl = L:L_shift:num_of_test_frames
                ...
                if(~flag_consecutive_bg)
                    ...
                    flag_cycle_complete = zeros(num_of_actions,L_M-L_dash+1); % oscillat
                    ...
                    for start_fr_num = 1:1:L_M-L_dash+1
                        for act_num = 1:num_of_actions
                            ...
                            % CRBM Codeword Classification using percentage
                            % of completion % larger the distance, the more
                            % closer the action is due to the complexity
                            dist_to_each_action(act_num,L_dash+start_fr_num-1)= w_perc(act_num,start_fr_num);
                    ...
                    % make a decision based on single frame
                    [mi_fr,id_fr] = max(dist_to_each_action(:,L_dash:L_M));
                    est_labels_per_frame{seq_row,seq_num} = id_fr;
            ...
            % the action class which contains a large percentage of action
            % is the true class
            [mi,id] = max(dist_of_temp_win); % since its the percentage of action present
...
%             end
    ...
    for seq_row = 1:1:num_of_actions
        for seq_num = 1:1:num_of_test_seqs
            ...
            est_class_per_frame = est_labels_per_frame{seq_row,seq_num};
            for l = 1:1:length(est_class_per_frame)
                results.action_labels_per_frame(seq_row,est_class_per_frame(l)) = results.action_labels_per_frame(seq_row,est_class_per_frame(l))+1;
            end
        end
    end
    ...
    results.action_labels_per_frame = results.action_labels_per_frame * 100 ./ (repmat(sum(results.action_labels_per_frame,2),1,num_of_actions) );
    
    fprintf('Test for Prediction Accuracy of CRBM Completed in %f hours\n',timeToTestCRBM/3600);
   
    results_phase3 = results;
    if(flag_full_seq)
        results_filename = sprintf('Results_Phase3_Action_PredictionAccuracy_Run%d_CRBMSizes_%d_nt_%d_fullseq.mat',rr,opts_crbm_per_action{1}.sizes(1),opts_crbm_per_action{1}.n{1});
        save(results_filename,'results_phase3');
    else
        results_filename = sprintf('Results_Phase3_Action_Classification_Run%d_L_%d.mat',rr,L);
        save(results_filename,'results_phase3');
    end
