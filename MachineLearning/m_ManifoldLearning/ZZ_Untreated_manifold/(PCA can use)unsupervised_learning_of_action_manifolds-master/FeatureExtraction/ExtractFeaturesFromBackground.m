        ...
        for set = 1:1:num_of_sets
            ...
            if(~isempty(bg_frame_num))   
                for k = bg_frame_num
                    ...
                    % generate a set of random samples of boxes
                    x_cor = size(img_mag,2)/4;
                    y_cor = size(img_mag,1)/4;
                    width = size(img_mag,2)/2;
                    height = size(img_mag,2)/2;
                    bound_box = [x_cor y_cor width height];
...
save('Background_Features_KTH.mat','Features');
