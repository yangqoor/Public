...
% 知乎    : slandarer
% -------------------------------------------------------------------------
% Zhaoxu Liu / slandarer (2025). radar chart 
% (https://www.mathworks.com/matlabcentral/fileexchange/126450-radar-chart), 
% MATLAB Central File Exchange. Retrieved September 4, 2025.
% =========================================================================
% 版本更新：
% -------------------------------------------------------------------------
% # version 2.0.0
% + 增添了背景所占的比例范围 
%   We can set the range for the background now.
% + 增添初始角度
%   We can set initial angle now.
% + 增添标签顺时针排布
%   Theta direction : 'normal'/'reverse'
    ...
    properties
        ...
        RRange = [0,1]; 
        Rotation = 0;
        ThetaDir = 'normal';      % 'normal' / 'reverse'
    ...
    methods
        ...
        function obj=draw(obj)
            ...
            XR=cos(tt').*((obj.RTick-obj.RLim(1))./diff(obj.RLim).*diff(obj.RRange) + obj.RRange(1)); XR=[XR;nan([1,length(obj.RTick)])];
            YR=sin(tt').*((obj.RTick-obj.RLim(1))./diff(obj.RLim).*diff(obj.RRange) + obj.RRange(1)); YR=[YR;nan([1,length(obj.RTick)])];
            obj.RTickHdl=plot(obj.ax,XR(:),YR(:),'Color',[200,200,200]./255,'LineWidth',1.1,'LineStyle','--');

            % 绘制雷达图
            for i=1:size(obj.XData,1)
                XP=cos(tn).*((obj.XData(i,:)-obj.RLim(1))./diff(obj.RLim).*diff(obj.RRange) + obj.RRange(1));
                YP=sin(tn).*((obj.XData(i,:)-obj.RLim(1))./diff(obj.RLim).*diff(obj.RRange) + obj.RRange(1));
                obj.PatchHdlP(i)=patch(obj.ax,XP,YP,obj.CList(mod(i-1,size(obj.CList,1))+1,:),...
                    'EdgeColor',obj.CList(mod(i-1,size(obj.CList,1))+1,:),'FaceAlpha',.2,...
                    'LineWidth',1.8);
                obj.PatchHdlL(i)=plot(obj.ax,[XP,XP(1)],[YP,YP(1)],...
                    'Color',obj.CList(mod(i-1,size(obj.CList,1))+1,:),'Marker','o',...
                    'LineWidth',1.8,'MarkerFaceColor',obj.CList(mod(i-1,size(obj.CList,1))+1,:));
                switch obj.Type
            ...
            for i=1:length(obj.RTick)
                obj.RLabelHdl(i)=text(obj.ax,cos(tnr).*((obj.RTick(i)-obj.RLim(1))./diff(obj.RLim).*diff(obj.RRange) + obj.RRange(1)),...
                                      sin(tnr).*((obj.RTick(i)-obj.RLim(1))./diff(obj.RLim).*diff(obj.RRange) + obj.RRange(1)),...
                                      obj.RTickLabelFormat(obj.RTick(i)),'FontName','Arial','FontSize',11);
            end
...
% =========================================================================
        ...
        function obj=setType(obj,Type)
            ...
            if ~isempty(findobj('Tag','SRClagend'))
                delete(findobj('Tag','SRClagend'))
                if isequal(obj.Type,'Patch')
