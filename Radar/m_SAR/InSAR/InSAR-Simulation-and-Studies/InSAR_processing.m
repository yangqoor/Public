...
%% 
% 载入原始数据及其中设定的参数。
load Cone_raw_data_A_B_20160122;
...
COL_min = 330;          % 左侧从第 330 列开始；
COL_max = 695;          % 右侧到第 695 列结束；
ROW_min = 248;          % 上侧从第 248 行开始；
ROW_max = 778;          % 下侧从第 778 行开始；
s_after_flat_earth_2 = s_after_flat_earth(ROW_min:ROW_max,COL_min:COL_max);
% 取第 330 列到第 695 列；取第 248 行到第 778 行；
...
if residue_count == 0   % 此时可以直接进行解缠绕，如上所述；
    ...
    %}
    Coherence_imag = calculate_coherence(s_imag_A(ROW_min:ROW_max,COL_min:COL_max),s_imag_B_after_CoRe(ROW_min:ROW_max,COL_min:COL_max));
...
[X,Y] = meshgrid(R0_RCMC(COL_min:COL_max),R_azimuth(ROW_min:ROW_max));
figure;
...
PHY_return_flat_earth = PHY_after_unwrapping + PHY_flat_earth(ROW_min:ROW_max,COL_min:COL_max);
...
%               —— 这一部分不太清楚怎么做。
%               —— 我计算过，但不是2*π的整数倍。     （待解决）
delta_PHY_reference = PHY_return_flat_earth(1,1 +1) - PHY_flat_earth(ROW_min,COL_min +1);
% [N1,N2] = size(PHY_return_flat_earth);
% delta_PHY_reference = PHY_return_flat_earth(round(N1/2),round(N2/2)) - PHY_flat_earth(ROW_min+round(N1/2)-1,COL_min+round(N2/2)-1);
% clear N1;clear N2;
...
% 下面进行计算：
R_1 = ones(ROW_max-ROW_min+1,1)*R0_RCMC(COL_min:COL_max);   % 用于高程计算的斜距。
...
% 这是斜距平面的高程信息，作图如下：
figure;
imagesc(R0_RCMC(COL_min:COL_max),R_azimuth(ROW_min:ROW_max),H_area);
title('斜距平面的高程信息');
xlabel('斜距坐标，单位：m');
ylabel('方位向坐标，y 轴，单位：m');
...
figure;imagesc(R0_RCMC(COL_min:COL_max),R_azimuth(ROW_min:ROW_max),H_error);
