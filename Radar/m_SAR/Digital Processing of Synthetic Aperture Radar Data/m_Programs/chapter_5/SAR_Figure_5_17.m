...
for k = 1:Ntarget
    ...
    S0 = S0+Wr.*Wa.*exp(-1i*4*pi*f0*R/C).*exp(1i*pi*Kr*(tau_m-2*R/C).^2);   % 生成回波信号矩阵 P156 (6.1)
end
...
title('（a）RD域频谱幅度');xlabel('距离向（采样点）');ylabel('方位向（采样点）');
subplot(2,2,2),imagesc(angle(Srd));axis image
title('（b）RD域频谱相位');xlabel('距离向（采样点）');
subplot(2,2,3),imagesc(abs(Sff));axis image
title('（c）二维频谱幅度');xlabel('距离向（采样点）');ylabel('方位向（采样点）');
subplot(2,2,4),imagesc(angle(Sff));axis image
title('（d）二维频谱相位');xlabel('距离向（采样点）');
