for itr_idx = 1:max_iter_num
    ...
    if itr_idx == 1
        % Calculate now depth_fine_vec from last frame depth_fine_vec
        depth_fine_vec = fun_dCoarse2FineDepth(d_depth_coarse_vecs{frm_idx,1}, ...
            depth_fine_vecs{frm_idx-1,1}, ProInfo);
        test_depth_fine_vec = depth_fine_vec;
        d_error = NaN;
    else
        % Calculate derv value
        cam_est_derv = fun_FineEstCamImageDerv(cam_est_vecmat, ...
            depth_fine_vec, ...
            ParaTable, ...
            CamInfo, ...
            ProInfo, ...
            EpiLine, ...
            ParaSet);
        norm_derv_vec = fun_FineErrorFunctionDerv(...
            cam_est_vecs(frm_sta:frm_end,1), ...
            cam_obs_vecs(frm_sta:frm_end,1), ...
            cam_est_derv, ...
            depth_coarse_vecs{frm_idx,1}, ...
            depth_fine_vec, ...
            theta, ...
            lambda, ...
            ParaSet);
    ...
    [test_cam_est_vecmat] = fun_EstCamImage(test_depth_fine_vec, ...
        ParaTable, ...
        CamInfo, ...
        ProInfo, ...
        EpiLine, ...
    ...
    error_value(frm_idx,itr_idx) = fun_FineErrorFunction(...
        test_cam_est_vec, ...
    ...
    if (itr_idx==1) || ((itr_idx>=2) && (d_error<0))
        ...
        cam_est_vec = test_cam_est_vec;
        cam_est_vecs{frm_idx,1} = cam_est_vec;
        depth_fine_vec = test_depth_fine_vec;
        depth_fine_vecs{frm_idx,1} = depth_fine_vec;
        fprintf('\tdE=%.2e\n', d_error);
        % Break
        if abs(d_error) < therd_fine
            break;
        end
    else
        % Reject
