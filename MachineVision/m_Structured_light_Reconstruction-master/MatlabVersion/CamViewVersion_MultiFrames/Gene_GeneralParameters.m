...
ParaSet.M = zeros(CamInfo.RANGE_HEIGHT*CamInfo.RANGE_WIDTH, 3);
ParaSet.D = zeros(CamInfo.RANGE_HEIGHT*CamInfo.RANGE_WIDTH, 3);
for c_idx = 1:CamInfo.RANGE_HEIGHT*CamInfo.RANGE_WIDTH
    x_cam = ParaSet.coord_cam(c_idx,2) - 1 + CamInfo.range_mat(1,1);
    y_cam = ParaSet.coord_cam(c_idx,1) - 1 + CamInfo.range_mat(2,1);
    tmp_vec = [(x_cam - CalibMat.cam(1,3))/CalibMat.cam(1,1); ...
        (y_cam - CalibMat.cam(2,3))/CalibMat.cam(2,2); ...
        1];
    ParaSet.M(c_idx,:) = (CalibMat.proMat(:, 1:3)*tmp_vec)';
    ParaSet.D(c_idx,:) = CalibMat.proMat(:, 4)';
end
% ParaSet.gauss = [0,1,1;
%     435.17, 1.471, 1.471;
%     709.79, 1.329, 1.329;
...
ParaSet.gradOpt = sparse(CamInfo.RANGE_HEIGHT*CamInfo.RANGE_WIDTH, ...
    CamInfo.RANGE_HEIGHT*CamInfo.RANGE_WIDTH);
for h = 1:CamInfo.RANGE_HEIGHT
    for w = 1:CamInfo.RANGE_WIDTH
        c_idx = (h-1)*CamInfo.RANGE_WIDTH + w;
        c_nbor = [];
        if h-1 >= 1
            c_nbor = [c_nbor; (h-1-1)*CamInfo.RANGE_WIDTH + w];
        end
        if h+1 <= CamInfo.RANGE_HEIGHT
            c_nbor = [c_nbor; (h+1-1)*CamInfo.RANGE_WIDTH + w];
        end
        if w-1 >= 1
            c_nbor = [c_nbor; (h-1)*CamInfo.RANGE_WIDTH + w-1];
        end
        if w+1 <= CamInfo.RANGE_WIDTH
            c_nbor = [c_nbor; (h-1)*CamInfo.RANGE_WIDTH + w+1];
        end
        ParaSet.gradOpt(c_idx, c_idx) = 4;
        ParaSet.gradOpt(c_idx, c_nbor) = -1 * 4 / size(c_nbor, 1);
    end
end
