%% 该程序在landweber14的基础上得到，主要修改了63行的限制条件，使输出能过为负数，使之能够用于增量landweber
...
function [im_deblur,MSE,stop_time,performance]=landweber14(bim,psf,iter,th,im_true,mth)
...
for k=1:iter
   ...
   %mth=max(x0(:))/7;
   x0(x0<mth(1))=mth(1);                               %limitation of positive
   x0(x0>mth(2))=mth(2);                               %limitation of positive
