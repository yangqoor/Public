...
opts.display = 0;
dirname = 'levin_dataset/results';
ssde_our = zeros(4,8);
k_size = [19, 17, 15, 27, 13, 21, 23, 23];
for i=1:4
    for j=1:8
        [i,j]
        img_outname=sprintf('%s/im%02d_ker%02d_out.png',dirname,i,j);
        ker_outname=sprintf('%s/im%02d_ker%02d_kernel.png',dirname,i,j);
        y = imread(sprintf('levin_dataset/im%02d_ker%02d.png',i,j));
        y = im2double(y);
        if size(y,3)==3
            yg = rgb2gray(y);
        else
