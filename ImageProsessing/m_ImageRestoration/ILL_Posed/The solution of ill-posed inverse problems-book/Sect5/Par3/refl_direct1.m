function [F,G]=refl_direct1(nz,R_ideal,nz1,nz2,n,k,h,ris0)
% 
% Вычисление коэф-та отражения системы по заданному распределению
% оптических длин nz*h^ R=R(k)=1-T(k). Вычисление невязки ||R(k)||.
...
if ris0(2)==1;F=norm(1-T-R_ideal)/sqrt(length(k));elseif ris0(2)==2;
F=norm(1-T-R_ideal,inf);else error('Неверно задано ris0');end
