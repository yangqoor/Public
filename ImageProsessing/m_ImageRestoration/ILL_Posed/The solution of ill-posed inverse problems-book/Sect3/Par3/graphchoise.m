...
subplot (2,2,2);cla;plot(log10(Alf(iz)),Opt(iz),'ro',log10(Alf(iv)),VV(iv)/max(VV),'bo',...
  log10(Alf),Opt, 'r.-',log10(Alf),VV/max(VV),'-b',log10(Alf),Del/max(VV),'-k');%
%xlabel('log_{10}(\alpha)');
...
hh=legend (s1,s2,s3,s4,s5,2);set(hh,'Position',[ 0.5 0.63 0.25 0.25 ]);
