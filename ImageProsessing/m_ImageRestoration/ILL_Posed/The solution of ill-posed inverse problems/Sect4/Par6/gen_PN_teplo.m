function [alf_opn,zgd]=gen_PN_teplo(t,A,u,U,V,sig,IL,X,y,w,delta,hdelta,C,q,NN,z);
% для Nonsatur
global meth
...
H=hdelta*norm(A);Delta=delta*norm(u);aaa=100;if (meth==2)|(meth==3);aaa=1000;elseif meth==4;
aaa=1e7;end

alf0=aaa*C*delta*norm(A)/(1-C*delta);
...
%figure(66);subplot(2,2,1);
%GDI=interp1(log10(Alf),gdis,log10(alf_opn));
%plot(log10(Alf),gdis,'r.-',[log10(Alf(1)) log10(Alf(end))],[0 0],log10(alf_opn),GDI,'o')
%subplot(2,2,2);
%GDI=interp1(log10(Alf),Opt,log10(alf_opt));
%plot(log10(Alf),Opt,'r.-',log10(alf_opt),GDI,'o');
%disp(['alf_opn = ' num2str(alf_opn) '  alf_opt = ' num2str(alf_opt)]);pause
...
set(gca,'FontName',FntNm,'YLim',[-0.1 1.1*max(z)]);
xlabel('t');ylabel('z(t), z^{\alpha_{opt}}(t)');title(srr(meth,:));
if meth==1;h7=legend('Точное реш.','Опт. выбор ',1);
set(h7,'Position',[ 0.607 0.829 0.244 0.087]);end
...
if meth==1;h7=legend('Точное реш.','ОПН ',1);
set(h7,'Position',[ 0.607 0.829 0.244 0.087]);end
