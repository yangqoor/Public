...
% Решение интегрального уравнения Фредгольма 1-го рода различными методами (c.181).
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
disp('  ');
disp('  Решение интегрального уравнения Фредгольма 1-го рода различными методами (c.181).');
...
zadan=input('  Введите номер решаемой задачи (1 -- регуляризация в W_1^1, 2 -- в W_1^2)(см.(c.181)):  ');
% Нахождение решения из W_1^1 - zadan=1; нахождение решения из W_1^2 - zadan=2
...
if zadan==1;disp(' ');disp('   Решается задача в пространстве W_1^1');disp(' ');
   a0=[ 7*sqrt(abs(s7-1))+5  5*ones(size(s3(1:end-1)))];
elseif zadan==2;disp(' ');disp('   Решается задача в пространстве W_1^2');disp(' '); 
   a0=[ 7*(abs(s7-1)).^(3/2)+5  5*ones(size(s3(1:end-1)))];
   else disp('    Неверный номер. Повторите ввод.');disp(' ');return; end
s=[ s7 s7(end)+s3(2:end)];
hsf=s(2)-s(1);% Шаг сетки решения
%
da0=diff(a0)./hsf;% da_0/dx
%figure(100);plot(s,a0);return
...
for ki=1:61;ff=K1(ki,:).*a0*hsf;vv(ki)=trapz(ff);end;u=vv;
...
disp(' ');if zadan==1;disp('   Тихоновская регуляризация задачи на классе W_2^1');
   ...
   disp('    Выбор параметров alpha');disp(' ');
...
if OMN==1;
disp('%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ');
disp(' ');if zadan==1;disp('   Обобщенный метод невязки (ОМН) в W_1^1');
disp('    для получения сходимости по вариации');else
disp('   Обобщенный метод невязки (ОМН) в W_1^2');
disp('    для получения сходимости по вариации первого порядка');end
