...
xlabel('t');ylabel('z(t), z^{\alpha}(t)');
  h7=legend('точное реш.','ОПН','L-кривая','ОПСФ','псевдоопт. выбор',1);
set(h7,'Position', [0.635952 0.788984 0.35 0.201095]);
hh=text(t(40),mst,'Отн. ошибки');set(hh,'FontName',FntNm);
hh=text(t(40),2*mst/3,['ОПН: ' num2str(erd)]);set(hh,'FontName',FntNm);
hh=text(t(40),2*mst/4,['L-кривая: ' num2str(erl)]);set(hh,'FontName',FntNm);
hh=text(t(40),2*mst/5, ['ОПСФ: ' num2str(ers)]);set(hh,'FontName',FntNm);
  hh=text(t(40),2*mst/7, ['псевдоопт:  ' num2str(erk)]);
