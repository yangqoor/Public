...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	/* general */
	mxArray* Ai_ptr = mxCreateNumericArray(2, dims_sharp, mxDOUBLE_CLASS, mxREAL);
	double* Ai      = mxGetPr(Ai_ptr);
	double gbari;
	/* exp_sqr_err */
	mxArray* exp_sqr_err_ptr = mxCreateNumericArray(2, dims_blurry, mxDOUBLE_CLASS, mxREAL);
	double* exp_sqr_err      = mxGetPr(exp_sqr_err_ptr);
	/* f1f2 */
	mxArray* f1f2_ptr = mxCreateNumericArray(2, dims_sharp, mxDOUBLE_CLASS, mxREAL);
	double* f1f2      = mxGetPr(f1f2_ptr);
	/* f2 */
	mxArray* f2_ptr = mxCreateNumericArray(2, dims_sharp, mxDOUBLE_CLASS, mxREAL);
	double* f2      = mxGetPr(f2_ptr);
	/* w1w2 */
	mxArray* w1w2_ptr = mxCreateDoubleMatrix(n_kernel, 1, mxREAL);
	double* w1w2      = mxGetPr(w1w2_ptr);
	/* w2 */
	mxArray* w2_ptr = mxCreateDoubleMatrix(n_kernel, 1, mxREAL);
  	double* w2      = mxGetPr(w2_ptr);
	/* mu1=sum(sum(Dp.*(D-mD))); */
	mxArray* mu1_ptr = mxCreateDoubleMatrix(1, 1, mxREAL);
	double* mu1      = mxGetPr(mu1_ptr);
	/* mu2 = data_points */
	mxArray* mu2_ptr = mxCreateDoubleMatrix(1, 1, mxREAL);
	double* mu2      = mxGetPr(mu2_ptr);
	...
	double *var_f      = mxGetPr(var_f_ptr);
	for(j=0; j<n_sharp; ++j)
