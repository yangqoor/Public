% [x,pad]=deconvL2NonUni_w_gradData(imblur,kernel,we,max_it,weight_x,weight_y,weight_xx,weight_yy,weight_xy,non_uniform = 0)
% [x,pad]=deconvL2NonUni_w_gradData(imblur,kernel,we,max_it,weight_x,weight_y,weight_xx,weight_yy,weight_xy,non_uniform = 0,saturation_mask,iminit,omega)
% [x,pad]=deconvL2NonUni_w_gradData(imblur,kernel,we,max_it,weight_x,weight_y,weight_xx,weight_yy,weight_xy,non_uniform = 1,saturation_mask,iminit,omega,theta_s,K,use_eff)
...
% Modification for non-uniform blur and gradient data term by Oliver Whyte, 2011

function [x,pad]=deconvL2NonUni_w_gradData(imblur,kernel,we,max_it,weight_x,weight_y,weight_xx,weight_yy,weight_xy,non_uniform,saturation_mask,iminit,omega,theta_s,K,use_eff)

if ~exist('omega','var'), omega = [1, 0.5, 0.25]; end
...
% Collect derivatives of g
DtBall = omega(1)*imblur + ...
         omega(2)*colourconv2(colourconv2(imblur,dxf ,'valid'),fliplr(flipud(dxf )),'full') + ...
         omega(2)*colourconv2(colourconv2(imblur,dyf ,'valid'),fliplr(flipud(dyf )),'full') + ...
         omega(3)*colourconv2(colourconv2(imblur,dxxf,'valid'),fliplr(flipud(dxxf)),'full') + ...
         omega(3)*colourconv2(colourconv2(imblur,dyyf,'valid'),fliplr(flipud(dyyf)),'full') + ...
         omega(3)*colourconv2(colourconv2(imblur,dxyf,'valid'),fliplr(flipud(dxyf)),'full');

ax = blurfn(x);
% Apply (Dx'*Dx + Dy'*Dy + Dxx'*Dxx + Dxy'Dxy + Dyy'*Dyy)
DtDax = omega(1)*ax ...
      + omega(2)*colourconv2(colourconv2(ax,dxf ,'valid'),fliplr(flipud(dxf )),'full') ...
      + omega(2)*colourconv2(colourconv2(ax,dyf ,'valid'),fliplr(flipud(dyf )),'full') ...
      + omega(3)*colourconv2(colourconv2(ax,dxxf,'valid'),fliplr(flipud(dxxf)),'full') ...
      + omega(3)*colourconv2(colourconv2(ax,dyyf,'valid'),fliplr(flipud(dyyf)),'full') ...
      + omega(3)*colourconv2(colourconv2(ax,dxyf,'valid'),fliplr(flipud(dxyf)),'full');
...
for iter = 1:max_it  
     ...
     % Apply (Dx'*Dx + Dy'*Dy + Dxx'*Dxx + Dxy'Dxy + Dyy'*Dyy)
     DtDAd = omega(1)*Ad ...
           + omega(2)*colourconv2(colourconv2(Ad,dxf ,'valid'),fliplr(flipud(dxf )),'full') ...
           + omega(2)*colourconv2(colourconv2(Ad,dyf ,'valid'),fliplr(flipud(dyf )),'full') ...
           + omega(3)*colourconv2(colourconv2(Ad,dxxf,'valid'),fliplr(flipud(dxxf)),'full') ...
           + omega(3)*colourconv2(colourconv2(Ad,dyyf,'valid'),fliplr(flipud(dyyf)),'full') ...
           + omega(3)*colourconv2(colourconv2(Ad,dxyf,'valid'),fliplr(flipud(dxyf)),'full');
     % Apply Atranspose
