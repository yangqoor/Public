...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	bool const clamp_edges_to_zero = 0 != mxGetScalar(prhs[6]); /* Set blurry pixels to zero if they involve sharp pixels outside the image? */
	bool const non_uniform         = 0 != mxGetScalar(prhs[7]); /* Non-uniform blur model? */

	mwSize const n_kernel = mxGetN(theta_list_ptr); /* Number of kernel elements */
	...
	mwSize const numdims     = mxGetNumberOfDimensions(imsharp_ptr);
	...
	mwSize const h_blurry    = (mwSize const)dims_blurry[0];
	mwSize const w_blurry    = (mwSize const)dims_blurry[1];
