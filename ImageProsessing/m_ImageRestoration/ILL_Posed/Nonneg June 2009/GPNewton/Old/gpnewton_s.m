  function [x_c,iter_hist] = gpnewton_s(x_c,opt_params,cost_params,out_params);

%  NOTE: This version of gpnewton allows for unpreconditioned iteration 
%  until the size of the active set is above a certain threshold.
%
...
%---------------------------------------------------------------------------
  ...
  switch_tol    = 2/3;%opt_params.switch_tol;   %  Preconditioning switch tol.
...
%---------------------------------------------------------------------------
    ...
    active_flag = (sum(Active(:))/ndim  > switch_tol);
    if precond_flag & active_flag
