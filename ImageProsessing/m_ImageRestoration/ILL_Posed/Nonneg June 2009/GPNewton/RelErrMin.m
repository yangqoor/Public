...
%---------------------------------------------------------------------
  ...
  %--------------------------------------------------------
  % Set the parameters for the cost functions
  %--------------------------------------------------------------
...
const = Data.Poiss_bkgrnd + Data.gaussian_stdev^2;
 ...
 WeightMat_large = spdiags(W(:),0,nx*ny,nx*ny);
  Cost_params.WeightMat = W;
  Cost_params.WeightMat_large = WeightMat_large;
%---------------------------------------------------------------------
%  Solve minimization problem.
%---------------------------------------------------------------------
  Cost_params.v_vec = randn(nx,ny);
 ...
 [alpha,fval,exitflag] = fminbnd(@(x) errmin_fun(x,f_0,Opt_params,Cost_params,Data),lhb,rhb,options)
