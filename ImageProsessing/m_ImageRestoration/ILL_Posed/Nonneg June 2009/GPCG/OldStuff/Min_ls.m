...
%------------------------------------------------------------------------
%  Set up and store cost function parameters in structure array Cost_params.
%------------------------------------------------------------------------
  ...
  if w_flag == 1  %  W = 1./([T*f_true]_i + bkgrnd + stdev^2);
    ...
    W = ones(nx,ny);
    Cost_params.weight            = W;    
    Cost_params.const             = Data.Poiss_bkgrnd + Data.gaussian_stdev^2;
    Cost_params.cost_fn           = 'ls_fun_new';
    Cost_params.hess_fn           = 'ls_hess2';
    my_fft                        = Cost_params.my_fft;
...
%------------------------------------------------------------------------
  ...
  if w_flag == 1 %| Data.type == 'toep'
    Opt_params.linesrch_fn       = 'linesrch_quad';
  else
    Opt_params.linesrch_fn       = 'linesrch_quad_opt_fft';
  end
