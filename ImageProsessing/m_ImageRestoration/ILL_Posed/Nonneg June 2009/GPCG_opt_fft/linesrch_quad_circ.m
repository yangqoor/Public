  function [x_new,phi_alpha,g_alpha,cost_params,ls_flag] = ...
       linesrch_quad_opt(x,p,g,q,q_fn,cost_params,opt_params,alpha_init)
  ...
  while ls_flag == 0
   ...
   % Check sufficient decrease condition.
    ...
    if phi_alpha <= phi_0 + mu*g(:)'*(x_new(:) - x(:)) 
      ...
      gLx_new = gam*L*x_new(:);
      ct_Fresid = conj(t_hat).*((1/const)*Fresid);
      cpu_t0 = cputime;
      iF_ct_Fresid = ifft2(ct_Fresid);
      TOTAL_FFT_TIME = TOTAL_FFT_TIME + (cputime - cpu_t0); 
      TOTAL_FFTS = TOTAL_FFTS + 1;      
      g_alpha = real(iF_ct_Fresid) + reshape(gLx_new,nx,ny);
      ls_flag = 3;
      return
    end
