  function [J,Params,g] = leastsquareslhd_fun(f,Params)
  
%  Evaluate penalized Poisson log likelihood cost functional
%      J(f) = .5||Tf-d||^2 + alpha/2*f'*L*f.
%    The gradient of J is given by 
%      g(f) = T'*(Tf-d) + alpha*L*f.
...
%  Declare global variables and initialize parameters and vectors.
  ...
  %------------  Compute J(f).  -----------------%
  resid = feval(Tmult,f,t_hat)-(d-b);
  Jfit = .5 * resid(:)' * ( Wmat * resid(:) );
  ...
  gfit = feval(Tmult,reshape(Wmat*resid(:),nx,ny),conj(t_hat));
