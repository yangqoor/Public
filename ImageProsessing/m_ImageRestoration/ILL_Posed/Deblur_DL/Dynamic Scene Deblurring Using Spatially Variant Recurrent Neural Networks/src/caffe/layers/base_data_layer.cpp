...
template <typename Dtype>
BasePrefetchingArbitraryDataLayer<Dtype>::BasePrefetchingArbitraryDataLayer(
    const LayerParameter& param)
    : BaseDataLayer<Dtype>(param),
      prefetch_free_(), prefetch_full_() {
  for (int i = 0; i < PREFETCH_COUNT; ++i) {
...
template <typename Dtype>
void BasePrefetchingArbitraryDataLayer<Dtype>::InternalThreadEntry() {
#ifndef CPU_ONLY
  cudaStream_t stream;
  if (Caffe::mode() == Caffe::GPU) {
    CUDA_CHECK(cudaStreamCreateWithFlags(&stream, cudaStreamNonBlocking));
  ...
  try {
    while (!must_stop()) {
      LiangjiBatch<Dtype>* batch = prefetch_free_.pop();
      load_batch(batch);
#ifndef CPU_ONLY
      if (Caffe::mode() == Caffe::GPU) {
        for(int i=0;i<batch->blobs_.size();i++)
	{
		batch->blobs_[i]->data().get()->async_gpu_push(stream);
	}
        CUDA_CHECK(cudaStreamSynchronize(stream));
      }
#endif
...
/////////////////////////////////
  for(int i=0;i<batch->blobs_.size();i++)
  {
      ...
      caffe_copy(batch->blobs_[i]->count(), batch->blobs_[i]->cpu_data(),
