classdef Net < handle
    ...
    methods
        ...
        % SF: add
        function weights = get_weights(self)
            weights = caffe_('get_weights', self.hNet_self);
        end
        function set_weights(self, weights)
            caffe_('set_weights', self.hNet_self, weights);
        end
        function data = get_data(self)
            data = caffe_('get_all_data', self.hNet_self);
        end
        function diff = get_diff(self)
            diff = caffe_('get_all_diff', self.hNet_self);
        end
        %
        ...
        function res = forward_csize(self, input_data, shape)
            CHECK(iscell(input_data), 'input_data must be a cell array');
            CHECK(length(input_data) == length(self.inputs), ...
                'input data cell length must match input blob number');
            % resize input and copy data to input blobs
            for n = 1:length(self.inputs)
                self.blobs(self.inputs{n}).reshape(shape{n});
                self.blobs(self.inputs{n}).set_data(input_data{n});
            end
            self.forward_prefilled();
            % retrieve data from output blobs
