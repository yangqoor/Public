function [padimg, ypadpre, ypadpost,xpadpre, xpadpost] = myPadImgforDeblur1(FBImg, kernel2d)
% tic;
...
%         cur_padimg(1:end-ypad,1:end-xpad,:) = bimg;
    ...
    for ch = 1:size(cur_padimg,3)
        for i = 1:10
%             cur_FImg = myPadDeconvL2_PreComp(cur_padimg(:,:,ch), cur_ker, wt_ireg, ConstArgs);
%             cur_BImg = myReblurL2_PreComp(cur_FImg, cur_ker, wt_breg, ConstArgs);
            
            cur_BImg = myPadDeconvReblurL2(cur_padimg(:,:,ch), cur_ker, wt_ireg, wt_breg, ConstArgs);
