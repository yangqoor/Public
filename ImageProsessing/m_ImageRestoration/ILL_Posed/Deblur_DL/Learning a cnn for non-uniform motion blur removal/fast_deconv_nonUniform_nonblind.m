function [yout] = fast_deconv_nonUniform_nonblind(yin, yout0, kernels, params, kernelInit)
%
%
...
% load parameters
%lambda = params.lambda; %2e+3;
alpha = params.alpha; %1/2;
...
Outiter = 1;
beta = 1;
%[grad_prob_mag_samples, grad_prob_ori_samples] = grad_prob(kernels.kernels_samples_mag, kernels.kernels_samples_ori, kernels.kernels_prob, [m, n]);
%kernels.grad_prob_mag_samples = grad_prob_mag_samples;
%kernels.grad_prob_ori_samples = grad_prob_ori_samples;
beta = params.beta;
mu = params.mu;
gamma = params.gamma;
...
while beta < beta_max & Outiter <= 1;% params.maxIter_out
    %% update the gradients
   [bmag, bori] = motion2magori(bmu, bmv);
   for c = 1 : d
        yout = yin_orig(:,:,c);
        [yout_d1, yout_d2, youtx, youty] = compDircDeri_1ord(yout, bmu_nor, bmv_nor);
        yin = yin_orig(:,:,c);
        by = motionBlurConv_mirror(double(yin), bmag, bori); % ??? to be updated
        beta = gamma;
        for k = 1 : params.maxIter_in
          %Outiter = Outiter + 1; 
          fprintf('\n Outer iteration %d; Inner iteration %d for optimization of sharp image; beta %.3g\n',Outiter, k, beta);

          % z-subproblem
          eps = 1e-6;
          gamma_nonunif = gamma * sqrt(bmag); % max(gamma * sqrt(bmag), eps);
          zx = softThresh(youtx, gamma_nonunif ./ beta); 
          zy = softThresh(youty, gamma_nonunif ./ beta);
          
%          zx = pValThresh(youtx,  gamma_nonunif ./ beta, params.alpha); 
%          zy = pValThresh(youty, gamma_nonunif ./ beta, params.alpha);

          % x-subproblem
          zxx = -[zx(:,2) - zx(:, 1), -diff(zx,1,2)]; 
          zyy = -[zy(2,:) - zy(1, :); -diff(zy,1,1)]; 

          b = by - beta * (zxx + zyy);
          yout = bicg(@(x,tflag) Afun(x,bmag, bori, beta, tflag),b(:),1e-5,300,[],[],yout(:));
          yout = reshape(yout, m, n);

          % update the gradient terms with new solution
          youtx = [diff(yout, 1, 2), yout(:,n-1) - yout(:,n)]; 
          youty = [diff(yout, 1, 1); yout(m-1,:) - yout(m,:)]; 
