function k_out = pcg_kernel_irls_conv(k_init, X, Y, opts,w_matrix)
...
for i = 1:length(X)
  ...
  rhs{i} = conv2(flipX{i}, w_matrix.*Y{i}, 'valid');
end;
...
for iter = 1 : 1
  ...
  k_out = local_cg(k_prev, X, flipX, ks,weights_l1, rhs, pcg_tol, pcg_its,w_matrix);
end;
  
% local implementation of CG to solve the reweighted least squares problem
function k = local_cg(k, X, flipX, ks, weights_l1, rhs, tol, max_its,w_matrix)

Ak = pcg_kernel_core_irls_conv(k, X, flipX, ks,weights_l1,w_matrix);
...
for iter = 1:max_its
  ...
  Ap = pcg_kernel_core_irls_conv(p, X, flipX, ks, weights_l1,w_matrix);
