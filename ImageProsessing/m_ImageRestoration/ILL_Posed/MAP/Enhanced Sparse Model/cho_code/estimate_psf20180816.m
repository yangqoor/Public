function psf = estimate_psf20180816(blurred_x, blurred_y, latent_x, latent_y, weight, psf_size)
...
%     blurred_yf = dy_f .* blurred_f; %% FFT (By)
    ...
    % initialize psf
    psf_prev = ones(psf_size) / prod(psf_size);
    
    % SOLVE PSF using IRLS and CG method
    for i = 1:10
        psf = conjgrad20180818(psf_prev, b, 20, 1e-5, @compute_Ax, p);
        psf_prev = psf;
    end
    
    
    psf(psf < 0) = 0; %max(psf(:))*0.05
    psf = psf / sum(psf(:));    
end
