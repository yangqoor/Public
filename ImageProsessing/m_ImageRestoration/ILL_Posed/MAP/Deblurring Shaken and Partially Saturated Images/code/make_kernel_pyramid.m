        ...
        if s==1
			...
			if isempty(init_tt)
				...
				for k=1:size(init_tt,2)
					...
					csf = csf + reshape(exp(-dists_sqr/2),size(ttx_s));
				end	
				pyr_kernel{s} = csf;
			end
        else
			if non_uniform
				if chain_downsampling
                    % pyr_kernel{s} = downsample_kernel(pyr_kernel{s},pyr_tt(s,:),pyr_kernel{s-1},pyr_tt(s-1,:),scale_ratio_k,tgs,s,s-1);
                    pyr_kernel{s} = downsample_kernel_imresize(pyr_kernel{s},pyr_tt(s,:),pyr_kernel{s-1},pyr_tt(s-1,:),scale_ratio_k,tgs,s,s-1);
				else
                    % pyr_kernel{s} = downsample_kernel(pyr_kernel{s},pyr_tt(s,:),pyr_kernel{1},pyr_tt(1,:),scale_ratio_k,tgs,s,1);
                    pyr_kernel{s} = downsample_kernel_imresize(pyr_kernel{s},pyr_tt(s,:),pyr_kernel{1},pyr_tt(1,:),scale_ratio_k,tgs,s,1);
				end
			else
			    error('need to implement new downsampling for uniform kernels')
				if chain_downsampling
					pyr_kernel{s} = downsample_kernel(pyr_kernel{s}, {pyr_tt{s,1}/scale_ratio_k,pyr_tt{s,2}/scale_ratio_k,pyr_tt{s,3}/scale_ratio_k},pyr_kernel{s-1},pyr_tt(s-1,:),scale_ratio_k,tgs,s,s-1);
				else
					pyr_kernel{s} = downsample_kernel(pyr_kernel{s}, {pyr_tt{s,1}/scale_factor,pyr_tt{s,2}/scale_factor,pyr_tt{s,3}/scale_factor},pyr_kernel{1},pyr_tt(1,:),scale_ratio_k,tgs,s,1);
				end
			end
        end
