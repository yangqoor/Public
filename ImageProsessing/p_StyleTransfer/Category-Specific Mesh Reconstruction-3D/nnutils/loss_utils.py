...
def mask_dt_loss(proj_verts, dist_transf):
    ...
    dist_transf = torch.nn.functional.grid_sample(
        dist_transf, sample_grid, padding_mode='border', align_corners=True)
...
def triangle_loss(verts, edge2verts):
    ...
    Returns:
        loss : scalar
    """
    indices_repeat = torch.stack(
        [edge2verts, edge2verts, edge2verts], dim=2)  # B X E X 3 X 4
...
class EdgeLoss(object):
    ...
    def __init__(self, verts, edges2verts, margin=2, use_bad_edge=False, use_l2=False):
        ...
        self.zeros = torch.autograd.Variable(
            torch.zeros(1).cuda(), requires_grad=False)
