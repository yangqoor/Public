...
class Finetune():
    ...
    def finetune(self):
        ...
        for batch, (unlabel_lr, unlabel_gt, idx) in enumerate(self.loader_ft):
            ...
            R_re_un, L_re_un, _ = self.model_re(r_un.detach(), l_un.detach(), self.alpha, unlabel_lr)
            R_re_un_o, L_re_un_o, _ = model_re_o(r_un, l_un, self.alpha, unlabel_lr)
        ...
        with torch.no_grad():
            for idx_scale, scale in enumerate(self.scale):
                ...
                for idx_img, (lr, hr, filename) in enumerate(tqdm_test):
                    ...
                    R_l_re, L_l_re, _ = self.model_re(ListR_l[-1], ListL_l[-1], alpha, lr)
                    Is_l = R_l_re * L_l_re
