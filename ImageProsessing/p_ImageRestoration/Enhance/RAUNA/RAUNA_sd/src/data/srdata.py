...
class SRData(data.Dataset):
    def __init__(self, args, name='', train=True, benchmark=False):
        ...
        if train:
            ...
            if args.test_only and len(data_range) == 1:
                data_range = data_range[0]
            elif args.finetune and len(data_range) == 1:
                data_range = data_range[0]
            else:
                data_range = data_range[1]
        self.begin, self.end = list(map(lambda x: int(x), data_range)) #把data range中的每一个数字都转成int

        self._set_filesystem(args.dir_data) #arg.dir_date='../data/'
        if args.ext.find('img') < 0: #寻找ext中是否含有‘img’  在判断data的类型
            path_bin = os.path.join(self.apath, 'bin')
            os.makedirs(path_bin, exist_ok=True)


        list_hr, list_lr= self._scan()
       # print('use this twice')
        #print('len_list_hr:', len(list_hr))
        #print('use this')
        if args.ext.find('bin') >= 0:   #args.ext = 'sep'
    ...
    def _scan(self):
        names_hr = sorted(
            glob.glob(os.path.join(self.dir_hr, '*' + self.ext[0]))  #glob 返回所有匹配文件名list
        )
        #print('len_hr:', len(names_hr))
        names_lr = [[] for _ in self.scale]
        #print('len_lr:',len(names_lr))
        for f in names_hr:
            filename, _ = os.path.splitext(os.path.basename(f)) #返回文件名
    ...
    def __getitem__(self, idx):
        ...
        )  #args.rgb_range=255 so it does nothing with 255
