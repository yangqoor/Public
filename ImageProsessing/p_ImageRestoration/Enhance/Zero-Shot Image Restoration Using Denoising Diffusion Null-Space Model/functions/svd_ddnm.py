...
def ddnm_diffusion(x, model, b, eta, A_funcs, y, cls_fn=None, classes=None, config=None):
    """DDNM+扩散过程，基于零空间修正和时间旅行的零样本图像恢复
    ...
    with torch.no_grad():  # 推理阶段不计算梯度
        ...
        for i, j in tqdm(time_pairs):
            ...
            if j < i:  # 正常反向扩散步骤（对应论文Algo.1）
                ...
                if cls_fn is None:  # 无条件生成
                    et = model(xt, t)                  # 预测噪声ε_t（式7）
                else:               # 分类器引导生成
                    classes = classes.to('cuda')
                    classes = torch.ones(xt.size(0), dtype=torch.long, device=xt.device) * classes
                    et = model(xt, t, classes)         # 带类别的噪声预测
                    et = et[:, :3]
