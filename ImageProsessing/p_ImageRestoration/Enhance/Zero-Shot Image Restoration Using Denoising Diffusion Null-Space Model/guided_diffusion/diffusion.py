...
class Diffusion(object):
    ...
    def sample(self, simplified):
        ...
        if self.config.model.type == 'simple':
            # 加载简单结构模型（如CIFAR10、LSUN、CelebA_HQ）
            ...
            if self.config.data.dataset == "CIFAR10":
                ...
                # 自动下载CelebA_HQ检查点（论文3.2节提到的预训练模型）
                if not os.path.exists(ckpt):
                    download('https://image-editing-test-12345.s3-us-west-2.amazonaws.com/checkpoints/celeba_hq.ckpt', ckpt)
            else:
                raise ValueError
            
            # 加载模型权重并部署到多GPU（论文4.1节实验设置）
            ...
            # 如果启用分类器引导，加载分类器并定义梯度函数（对应论文3.1节条件生成）
            if self.config.model.class_cond:
                config_dict = vars(self.config.classifier)
                classifier = create_classifier(**config_dict)
                classifier_ckpt = os.path.join(self.args.exp, f'logs/imagenet/{self.config.data.image_size}x{self.config.data.image_size}_classifier.pt')
                classifier.load_state_dict(torch.load(classifier_ckpt, map_location=self.device))
                ...
                def cond_fn(x, t, y):
                    with torch.enable_grad():
                        ...
                        # 计算梯度并缩放（对应论文classifier_scale参数）
                        return torch.autograd.grad(selected.sum(), x_in)[0] * self.config.classifier.classifier_scale
                
                cls_fn = cond_fn  # 将梯度函数传递给采样过程

        # 选择运行简化版或SVD版DDNM+（论文3.3节两种模式）
        if simplified:
            print(
                'Run Simplified DDNM (非SVD版本)',
                f'Sampling步数: {self.config.time_travel.T_sampling}',
                f'时间旅行长度: {self.config.time_travel.travel_length}',
                f'时间旅行重复次数: {self.config.time_travel.travel_repeat}',
                f'任务类型: {self.args.deg}'
            )
            self.simplified_ddnm_plus(model, cls_fn)  # 调用简化版算法（Algo.2）
        else:
            print(
                'Run SVD-based DDNM (标准版本)',
                f'Sampling步数: {self.config.time_travel.T_sampling}',
                f'时间旅行长度: {self.config.time_travel.travel_length}',
                f'时间旅行重复次数: {self.config.time_travel.travel_repeat}',
                f'任务类型: {self.args.deg}'
            )
            self.svd_based_ddnm_plus(model, cls_fn)  # 调用SVD版本（需奇异值分解）
    ...
    def simplified_ddnm_plus(self, model, cls_fn):
        ...
        g = torch.Generator()
        g.manual_seed(self.args.seed)  # 确保生成器使用相同的种子
        
        # 使用functools.partial预先绑定种子参数
