...
from models import outlier_net, unetmodel
...
class deblur(nn.Module):
    def __init__(self, maxit):
        ...
        self.maxit = maxit
        self.graddeblur = unetmodel.UNet(1, 1)
        self.outliermap = outlier_net.outlier(4, 1, 16, 3)
    ...
    def forward(self, x_init, blur, kernel, kernelT, reg):
        ...
        imgu = self.graddeblur(x_init)

        blur_out = torch.where(blur > 0.85, torch.full_like(blur, 0), torch.full_like(blur, 1))
        x_out = torch.where(x_init > 0.85, torch.full_like(x_init, 0), torch.full_like(x_init, 1))
        o1in = torch.cat((blur_out, x_out, blur, x_init), 1)
        outlier = self.outliermap(o1in)
        outlier = F.sigmoid(outlier)
        outlier = outlier * mask
        x = self.deconv_L2(x_init, blur, kernel, kernelT, outlier, imgu, reg=reg)
        return x
