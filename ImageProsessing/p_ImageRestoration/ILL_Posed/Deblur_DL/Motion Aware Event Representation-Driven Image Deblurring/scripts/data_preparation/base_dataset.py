...
class BaseVoxelDataset(Dataset):
    ...
    def __init__(self, data_path, transforms={}, sensor_resolution=None, num_bins=10,
                 ...
                 return_flow=False, return_prev_flow=False, voxel_temporal_bilinear=False, image_timestamps_path=None, keep_middle=False, return_format='torch'):
        """
    ...
    def __getitem__(self, index, seed=None):
        ...
        if self.return_voxelgrid:
            ...
            if self.voxel_method['method'] == 'ASYN_esim' or self.voxel_method['method']=='ASYN_real_data':
                voxel = self.get_events_asynchronous_accumulate_voxel_frame_center(xs, ys, ts, ps)
            if self.voxel_method['method'] == 'ASYNAV_esim' or self.voxel_method['method']=='ASYNAV_real_data':
                voxel = self.get_events_asynchronous_accumulate_avvoxel_frame_center(xs, ys, ts, ps) 
            # 上面的改完了
        ...
        if self.voxel_method['method'] == 'ASYN_esim':
            frame = self.get_frame(index)
            frame_gt = self.get_gt_frame(index)
            frame = self.transform_frame(frame, seed, transpose_to_CHW=True) # to tensor
            frame_gt = self.transform_frame(frame_gt, seed, transpose_to_CHW=True)
        ...
        if self.voxel_method['method'] == 'ASYNAV_esim':
            frame = self.get_frame(index)
            frame_gt = self.get_gt_frame(index)
            frame = self.transform_frame(frame, seed, transpose_to_CHW=True) # to tensor
            frame_gt = self.transform_frame(frame_gt, seed, transpose_to_CHW=True)
        ...
        elif self.voxel_method['method'] == 'ASYN_real_data':
            frame = self.get_frame(index)
            frame = self.transform_frame(frame, seed, transpose_to_CHW=True)  # to tensor
            if self.return_gt_frame:
                frame_gt = self.get_gt_frame(index)
        ...
        elif self.voxel_method['method'] == 'ASYNAV_real_data':
            frame = self.get_frame(index)
            frame = self.transform_frame(frame, seed, transpose_to_CHW=True)  # to tensor
            if self.return_gt_frame:
                frame_gt = self.get_gt_frame(index)
    ...
    def set_voxel_method(self, voxel_method):
        ...
        elif self.voxel_method['method'] == 'ASYN_esim':
            self.length = self.num_frames - 2 # the first and the last age dont have events in both left and right
            self.event_indices = self.compute_frame_center_indeices()

        elif self.voxel_method['method'] == 'ASYNAV_esim':
            self.length = self.num_frames - 2 # the first and the last age dont have events in both left and right
            self.event_indices = self.compute_frame_center_indeices()
    ...
    def get_events_asynchronous_accumulate_voxel_frame_center(self, xs, ys, ts, ps):
        """
        Given events, return events accumulate voxel with frame centered
        The num_bins have to be even!
        @param xs tensor containg x coords of events
        ...
        @returns Voxel grid of input events
        """
        voxel_grid = events_to_asynaccumulate_voxel_torch(xs, ys, ts, ps, self.num_bins, sensor_size=self.sensor_resolution, keep_middle=self.keep_middle)
        # voxel_grid = events_to_accumulate_voxel_torch_2(xs, ys, ts, ps, self.num_bins, sensor_size=self.sensor_resolution)
        
        return voxel_grid

    def get_events_asynchronous_accumulate_avvoxel_frame_center(self, xs, ys, ts, ps):
        """
        Given events, return events accumulate voxel with frame centered
        The num_bins have to be even!
        @param xs tensor containg x coords of events
        ...
        @returns Voxel grid of input events
        """
        voxel_grid = events_to_asynaccumulate_avvoxel_torch(xs, ys, ts, ps, self.num_bins, sensor_size=self.sensor_resolution, keep_middle=self.keep_middle)
        # voxel_grid = events_to_accumulate_voxel_torch_2(xs, ys, ts, ps, self.num_bins, sensor_size=self.sensor_resolution)
...
def events_to_asynaccumulate_voxel_torch(xs, ys, ts, ps, B, device=None, sensor_size=(180, 240), keep_middle=False):
    ...
    for bi in range(int(B/2)):
        tstart = ts[0] + (dt/B)*bi
        beg = binary_search_torch_tensor(ts, 0, len(ts)-1, tstart)
        vb = asynevents_to_image_torch(xs[beg:end], ys[beg:end],
                ps[beg:end], ts[beg:end], device, sensor_size=sensor_size,
                clip_out_of_range=False)
        bins.append(-vb) # !
    # self
    if keep_middle:
...
def events_to_asynaccumulate_avvoxel_torch(xs, ys, ts, ps, B, device=None, sensor_size=(180, 240), keep_middle=False):
    ...
    for bi in range(int(B/2)):
        tstart = ts[0] + (dt/B)*bi
        beg = binary_search_torch_tensor(ts, 0, len(ts)-1, tstart)
        vb = asynevents_to_avimage_torch(xs[beg:end], ys[beg:end],
                ps[beg:end], ts[beg:end], device, sensor_size=sensor_size,
                clip_out_of_range=False)
        bins.append(-vb) # !
    # self
...
# 将这里改成异步累积应该就可以
def asynevents_to_image_torch(xs, ys, ps,ts,
        device=None, sensor_size=(180, 240), clip_out_of_range=True,
        interpolation=None, padding=True, default=0):
    """
    Method to turn event tensor to image but with asynchronous dealing. Allows for bilinear interpolation.
    @param xs Tensor of x coords of events
    @param ys Tensor of y coords of events
    @param ps Tensor of event polarities/weights
    @param device The device on which the image is. If none, set to events device
    ...
    img = (torch.ones(img_size)*default).to(device)
    timerec = (torch.zeros(img_size,dtype=torch.float64)*default).to(device)
    if interpolation == 'bilinear' and xs.dtype is not torch.long and xs.dtype is not torch.long:
        pxs = (xs.floor()).float()
        pys = (ys.floor()).float()
        dxs = (xs-pxs).float()
        ...
        try:
            mask = mask.long().to(device)
            xs, ys = xs*mask, ys*mask
            ts = (ts.to(torch.float64) * 1e-09).to(device)
            for j in range(len(ts)):
                img.index_put_((ys[j], xs[j]), img[ys[j]][xs[j]]*math.exp(-(ts[j]-timerec[ys[j]][xs[j]])/(5*1e-02))+ps[j], accumulate=False)
                timerec.index_put_((ys[j], xs[j]),ts[j])
            # img.index_put_((ys, xs), ps, accumulate=True)
            # print("able to put tensor {} positions ({}, {}) into {}. Range = {},{}".format(
            #     ps.shape, ys.shape, xs.shape, img.shape,  torch.max(ys), torch.max(xs)))
        except Exception as e:
...
def asynevents_to_avimage_torch(xs, ys, ps,ts,
        device=None, sensor_size=(180, 240), clip_out_of_range=True,
        interpolation=None, padding=True, default=0):
    """
    Method to turn event tensor to image but with asynchronous dealing. Allows for bilinear interpolation.
    ...
    img = (torch.ones(img_size)*default).to(device)
    last_state = (torch.zeros(img_size).long()).to(device)
    if xs.dtype is not torch.long:
        xs = xs.long().to(device)
    if ys.dtype is not torch.long:
        ys = ys.long().to(device)
    if ps.dtype is not torch.long:
        ps = ps.long().to(device)
    try:
        flag = torch.ones_like(ps).long().to(device)
        times = (torch.zeros(img_size).long()).to(device)
        times.index_put_((ys, xs), flag, accumulate=True)
        times[times == 0] = 1

        for j in range(len(ts)):
            last_state.index_put_((ys[j],xs[j]), ps[j], accumulate=True)
            img.index_put_((ys[j],xs[j]), torch.exp(last_state[ys[j]][xs[j]]*0.2), accumulate=True)
        av_img = img/times
    except Exception as e:
        print("Unable to put tensor {} positions ({}, {}) into {}. Range = {},{}".format(
            ps.shape, ys.shape, xs.shape, img.shape,  torch.max(ys), torch.max(xs)))
