...
class KernelEstimationModel(BaseModel):
    """Base model for blur kernel estimation"""

    def __init__(self, opt):
        super(KernelEstimationModel, self).__init__(opt)
    ...
    def optimize_parameters(self, current_iter):
        ...
        if self.opt['datasets']['train'].get('use_gt', False):
            preds = self.net_g(self.lq, event = self.voxel, mask = self.mask, gt=self.gt)
        ...
        if self.cri_pix:
            ...
            if self.pixel_type == 'PSNRATLoss':
                l_pix += self.cri_pix(*preds, self.lq)

            elif self.pixel_type == 'PSNRGateLoss':
                for pred in preds:
                    l_pix += self.cri_pix(pred, self.lq, self.mask)

            elif self.pixel_type == 'PSNRLoss':
                for pred in preds:
                    l_pix += self.cri_pix(pred, self.lq)
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            while i < n:
                ...
                if self.opt['datasets']['val'].get('use_gt'):
                    pred = self.net_g(x = self.lq[i:j, :, :, :], event = self.voxel[i:j, :, :, :], mask = self.mask[i:j, :, :, :],gt=self.gt[i:j, :, :, :])
                else:
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger,
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            raw_img = tensor2img([visuals['lq']], rgb2bgr=rgb2bgr)
            if 'gt' in visuals:
                ...
                if use_image:
                    for name, opt_ in opt_metric.items():
                        ...
                        self.metric_results[name] += getattr(
                            metric_module, metric_type)(sr_img, raw_img, **opt_)
                else:
                    for name, opt_ in opt_metric.items():
                        metric_type = opt_.pop('type')
                        self.metric_results[name] += getattr(
