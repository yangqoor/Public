...
class ImageEventGreyRestorationModel(BaseModel):
    """Base Event-based deblur model for single image deblur."""

    def __init__(self, opt):
        super(ImageEventGreyRestorationModel, self).__init__(opt)
    ...
    def setup_optimizers(self):
        ...
        for k, v in self.net_g.named_parameters():
            if v.requires_grad:
                if k.startswith('evencoder') or k.startswith('module.dcns'):
    ...
    def optimize_parameters(self, current_iter):
        ...
        self.gt = self.gt[:,0,:,:]*0.11+self.gt[:,1,:,:]*0.59+self.gt[:,2,:,:]*0.3
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger,
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            if 'gt' in visuals:
                ...
                gt_img = visuals['gt'][:,0,:,:]*0.11+visuals['gt'][:,1,:,:]*0.59+visuals['gt'][:,2,:,:]*0.3
