...
function boxDimension = boxcount(c,varargin)
...
%% 画图
%a=mean(y)-b*mean(x) ; %a为截距
%b=lxy/lxx ; 
%xx=linspace(0,4,800); 
% 4的值根据a/b的值定，去离其最近的整数值；800的值是数据个数，可以根据情况随意定
%yy=a+b*xx; 
%hold on 
%plot(xx,yy,'b-') 
%text(1,7,'FitFunction: y=a+b*x') 
%text(1,8,'分形计盒维数直线方程：') 
%text(1,9,strcat('FitFunction: y=',num2str(b), '*x ', '+',num2str(a))); 
